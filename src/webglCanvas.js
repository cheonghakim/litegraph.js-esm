...
function enableWebGLCanvas(canvas, options) {
  ...
  WebGLCanvasGradient.prototype.toTexture = function () {
    ...
    //fill the remaining
    if (i < 128) for (var j = i; j < 128; j += 1) buffer.set(current[1], j * 4);
  ...
  ctx.stroke = function () {
    ...
    if (this.lineWidth * vec2.length(tmp_vec2) <= 1.0) return this.strokeThin();
  ...
  ctx.fillText = ctx.strokeText = function (text, startx, starty) {
    ...
    function renderCurrentBatch() {
      if (vertices_index === 0 || coords_index === 0 || currentPage === -1) {
      ...
      uniforms.u_transform = this._matrix;
      uniforms.u_viewport = viewport;
      if (!uniforms.u_angle_sincos) uniforms.u_angle_sincos = vec2.create();
    ...
    else if (this.textAlign === "center") offset = (x + point_size * 0.5) * 0.5;
    if (offset) {
  ...
  function createFontAtlas(
    ...
    // 공간 문자 처리
    info.space = info.kernings[" "] ? info.kernings[" "].width / fontSize : 0;
...
function debugTextures(texturesArray) {
 ...
 */
async function makeTexture(textureContainer) {
  ...
  texture.texture_type = textureContainer.texture_type || texture.texture_type;
 ...
 * texture -> base64
 */
function textureToBase64(glTexture) {
 ...
 */
async function saveComplexData(data, expiryInDays = 1) {
  try {

...
export class LiteGraph {
    ...
    static extendClass(target, origin) {
        ...
        if (origin.prototype) {
            for (const i in origin.prototype) {
                if (
                    !Object.prototype.hasOwnProperty.call(origin.prototype, i)
                ) {
                    continue;
                }
    ...
    static pointerListenerAdd(oDOM, sEvIn, fCall, capture = false) {
        ...
        if (pointerEvents.includes(sEvent)) {
            if (
                sMethod !== "mouse" ||
                ["down", "up", "move", "over", "out", "enter"].includes(sEvent)
            ) {
            ...
            if (base_class.supported_extensions) {
                for (let i in base_class.supported_extensions) {
                    ...
                    if (ext && ext.constructor === String) {
                        this.node_types_by_file_extension[ext.toLowerCase()] =
                            base_class;
                    }
                }
            }
...
export class LGraphCanvas {
    ...
    async enableWebGL() {
        ...
        const webglCtx = enableWebGLCanvas(this.canvas);
    ...
    drawNode(node, ctx) {
        ...
        const titleHeight = title_mode ? 0 : LiteGraph.NODE_TITLE_HEIGHT;
        ...
        if (node.flags.collapsed) {
            ...
            if (title != null) {
                node._collapsed_width = Math.min(
                    node.size[0],
                    ctx.measureText(title).width + titleHeight * 2
            ...
            if (node.inputs) {
                for (var i = 0; i < node.inputs.length; i++) {
                    ...
                    if (
                        ...
                        // 슬롯 색상 변경 - 동그란 슬롯으로 변경
                        // if (this.read_only) ctx.fillStyle = "rgba(0, 47, 255, 0)";
                        // else
                        // ctx.fillStyle = "rgba(76, 76, 76, 0.7)";
                    ...
                    if (render_text) {
                        ...
                        if (text) {
                            ...
                            if (horizontal || slot.dir == LiteGraph.UP) {
                                ...
                                if (this.options.useWebgl) {
                                    ctx.fillText(text, pos[0] + 10, pos[1] + 2);
                                } else {
    ...
    drawNodeShape(node, ctx, size, fgcolor, bgcolor, selected, mouse_over) {
        ...
        var area = tmp_area;
        // area[0] = 0; //x
        // area[1] = render_title ? -titleHeight : 0; //y
        // area[2] = size[0] + 1; //w
        // area[3] = render_title ? size[1] + titleHeight : size[1]; //h
        ...
        //if(node.flags.collapsed)
        {
            ...
            //   ctx.fillRect(0, titleHeight, area[2], 2);
            // }
        }
        ctx.shadowColor = "transparent";
        ...
        if (render_title || title_mode == LiteGraph.TRANSPARENT_TITLE) {
            ...
            ctx.font = "Bold " + this.node_title_font_size + "px Arial";
            //title text
            ...
            if (!low_quality) {
                ...
                if (title) {
                    ...
                    let height = 0;
                    // 타이틀 다음줄로 이어 쓰기, 8자 까지만 표시
                    let y = LiteGraph.NODE_TITLE_TEXT_Y - titleHeight;
                    let lineHeight = 15;
                    const maxWidth = node.size[0] - 5;
                    const words = title.split("");
                    let line = "";

                    // 세로로 높이를 추가 -> 너무 안예뻐서 바꿈
                    ...
                    for (let n = 0; n < words.length; n++) {
                        ...
                        const metrics = ctx.measureText(testLine);
                        const testWidth = metrics.width;
                        if (testWidth > maxWidth - titleX * 2 && n > 0) {
                            // ctx.fillText(line + "..", titleX, y);
                            line += "..";
                            break;
                        } else {
                    ...
                    if (node.flags.collapsed) {
                        ...
                        ctx.fillText(
                            line, //avoid urls too long
                            titleX, // + measure.width * 0.5,
                            y
                        );
                        ctx.textAlign = "left";
                    } else {
    ...
    drawGroups(canvas, ctx) {
        ...
        for (let i = 0; i < groups.length; ++i) {
            ...
            if (
                !LiteGraph.overlapBounding(this.visible_area, group._bounding)
            ) {
                continue;
            } //out of the visible area
            ...
            if (!this.read_only) {
                ...
                ctx.fillText(line, pos[0] + 10, pos[1] + group.size[1] - 12);
            }
                    ...
                    if (options.do_type_filter && !opts.skipFilter) {
                        ...
                        if (sOut && _sV) {
                            ...
                            if (
                                ...
                                LiteGraph.registered_slot_out_types[_sV].nodes
                            ) {
                                // type is stored
    ...
    createPanel(title, options) {
        ...
        root.addWidget = (type, name, value, options, callback) => {
            ...
            if (type == "code")
                ...
                input.value = type === "string" ? str_value : Number(str_value);
                ...
                elem.querySelector(".property_name").innerText =
                    options.label || name;
                const value_element = elem.querySelector(".property_value_img");
    ...
    showShowNodePanel(node) {
        ...
        function inner_refresh() {
            ...
            for (let pName in node.properties) {
                if (exceptions.includes(pName) || pName.includes("editable-"))
                    continue;
            ...
            for (let i in nodeInfo) {
                ...
                if (otherRoundX === roundX) {
                    if (roundY < otherRoundY) {
                        ...
                        ctx.beginPath();
                        ctx.moveTo(otherRoundX, otherRoundY);
                        ctx.lineTo(roundX, roundY + roundHeight + titleHeight);
                        ctx.stroke(); // 선을 그리기 위해 stroke 호출
                    }
                }

    ...
    LGraphCanvas.prototype.renderLink = function (
        ...
        if (flow) {
            ...
            for (var i = 0; i < 5; ++i) {
                ...
                // When the render mode is LiteGraph.STRAIGHT_LINK
                if (this.links_render_mode === LiteGraph.STRAIGHT_LINK) {
                    // Calculate the path of the bent straight line
                    var start_x = a[0];
                    var start_y = a[1];
                    var end_x = b[0];
                    var end_y = b[1];

                    // Adjustments based on the starting direction
                    if (start_dir == LiteGraph.RIGHT) {
                        start_x += 10; // When the line should exit to the right
                    } else if (start_dir == LiteGraph.LEFT) {
                        start_x -= 10; // When the line should exit to the left
                    } else if (start_dir == LiteGraph.DOWN) {
                        start_y += 10; // When the line should exit downward
                    } else if (start_dir == LiteGraph.UP) {
                        start_y -= 10; // When the line should exit upward
                    }

                    // Adjustments based on the ending direction
                    if (end_dir == LiteGraph.LEFT) {
                        end_x -= 10; // When the line ends to the left
                    } else if (end_dir == LiteGraph.RIGHT) {
                        end_x += 10; // When the line ends to the right
                    } else if (end_dir == LiteGraph.UP) {
                        end_y -= 10; // When the line ends upward
                    } else if (end_dir == LiteGraph.DOWN) {
                        end_y += 10; // When the line ends downward
                    }

                    // Calculate the midpoint (bending point)
                    var mid_x = (start_x + end_x) * 0.5; // Horizontal midpoint
                    var mid_y = (start_y + end_y) * 0.5; // Vertical midpoint (can be used if needed)

                    // Calculate the length of each segment
                    var segment1_length = Math.abs(mid_x - start_x); // First horizontal segment
                    var segment2_length = Math.abs(end_y - start_y); // Vertical segment
                    var segment3_length = Math.abs(end_x - mid_x); // Final horizontal segment
                    ...
                    var progress = f * total_length; // Animation progress

                    var pos = [start_x, start_y]; // Starting point

                    // Animation in the first horizontal segment
                    ...
                    // Animation in the second vertical segment (upward/downward movement)
                    else if (progress <= segment1_length + segment2_length) {
                        var remaining = progress - segment1_length;

                        // Determine whether to move up or down
                        if (start_y < end_y) {
                            pos = [mid_x, start_y + remaining]; // Move downward
                        } else {
                            pos = [mid_x, start_y - remaining]; // Move upward
                        }
                    }
                    // Animation in the third horizontal segment (final horizontal movement)

    ...
    class LiteGraph {
        ...
        static pointerListenerAdd(oDOM, sEvIn, fCall, capture = false) {
            ...
            if (sMethod === "pointer" && !window.PointerEvent) {
                ...
                if (mapping[sEvent]) {
                    sMethod = "touch";
                    sEvent = mapping[sEvent];
                } else if (sEvent === "enter") {
                    console.log("debug: Should I send a move event?");
                } else {
                    console.warn(`Event ${sEvent} will not be called`);
                }
            }
            ...
            if (pointerEvents.includes(sEvent)) {
                if (
                    sMethod !== "mouse" ||
                    ["down", "up", "move", "over", "out", "enter"].includes(
                        sEvent
                    )
                ) {
            ...
            // fallback for touch/mouse/others
        ...
        static pointerListenerRemove(oDOM, sEvent, fCall, capture = false) {
            ...
            const method = LiteGraph.pointerevents_method;
            const sharedEvents = ["down", "up", "move", "over", "out", "enter"];
            ...
            if (sharedEvents.includes(sEvent)) {
            ...
            if (pointerOnlyEvents.includes(sEvent)) {
                if (method === "pointer") {
                    oDOM.removeEventListener(method + sEvent, fCall, capture);
                    return;
                }
    ...
    class LGraph {
        ...
        checkNodeTypes() {
            ...
            for (let i = 0; i < this._nodes.length; i++) {
                ...
                console.log(
                    "node being replaced by newer version: " + node.type
                );
    ...
    class LGraphNode {
        ...
        computeSize(out) {
            ...
            if (this.widgets_up) size[1] = Math.max(size[1], widgets_height);
                ...
                size[1] = Math.max(
                    size[1],
                    widgets_height + this.widgets_start_y
                );
         ...
         * @return {number_or_object} the slot (-1 if not found)
         */
        findOutputSlotFree(_optsIn) {
            const optsIn = _optsIn || {};
            const optsDef = { returnObj: false, typesNotAccepted: [] };
            const opts = Object.assign(optsDef, optsIn);
            if (!this.outputs) {
                return -1;
         ...
         */
        connectByType(slot, target_node, target_slotType, _optsIn) {
            const optsIn = _optsIn || {};
            const optsDef = {
            ...
            if (target_slot >= 0 && target_slot !== null) {
                ...
                if (
                    opts.createEventInCase &&
                    target_slotType == LiteGraph.EVENT
                ) {
                ...
                if (
                    ...
                    const target_slot = target_node.findInputSlotFree({
         ...
         * @return {Object} the link_info is created, otherwise null
         */
        connectByTypeOutput(slot, source_node, source_slotType, _optsIn) {
            const optsIn = _optsIn || {};
            const optsDef = {
                createEventInCase: true,
            ...
            if (source_slot >= 0 && source_slot !== null) {
                ...
                if (
                    opts.createEventInCase &&
                    source_slotType == LiteGraph.EVENT
                ) {
                    // WILL CREATE THE onExecuted OUT SLOT
                    if (LiteGraph.do_add_triggers_slots) {
                        const source_slot = source_node.addOnExecutedOutput();
                        return source_node.connect(source_slot, this, slot);
                    }
                }
                ...
                if (
                    ...
                    const source_slot = source_node.findOutputSlotFree({
                        typesNotAccepted: [LiteGraph.EVENT],
                    });
                    if (source_slot >= 0) {
                        return source_node.connect(source_slot, this, slot);
        ...
        getConnectionPos(is_input, slot_number, out) {
            ...
            if (this.horizontal) {
                out[0] =
                    this.pos[0] +
                    (slot_number + 0.5) * (this.size[0] / num_slots);
                if (is_input) {
                    out[1] = this.pos[1] - LiteGraph.NODE_TITLE_HEIGHT;
                } else {
    ...
    */
        ...
        _ctor(title) {
            ...
            Object.defineProperty(this, "pos", {
                set: function (v) {
                    ...
                    this._pos[0] = Math.round(v[0]);
                    this._pos[1] = Math.round(v[1]);
                },
                get: function () {
                    return this._pos;
                },
            ...
            Object.defineProperty(this, "size", {
                set: function (v) {
                    ...
                    this._size[0] = Math.max(140, Math.round(v[0]));
    ...
    class DragAndScale {
        ...
        onMouse(e) {
            ...
            if (
                e.type == LiteGraph.pointerevents_method + "down" &&
                is_inside
            ) {
                this.dragging = true;
    ...
    class LGraphCanvas {
        ...
        constructor(canvas, graph, options) {
            ...
            // undo/redo 스택
            this.maxStackSize = options.maxStackSize || 1000;
         ...
         */
        async setCanvas(canvas, skip_events) {
            const that = this;
        ...
        async enableWebGL() {
            ...
            const webglCtx = window.enableWebGLCanvas(this.canvas);
            if (webglCtx instanceof WebGLRenderingContext) {
                await webglCtx.loadCachedTextures();
                webglCtx.cacheFontAtlas("Arial", "normal");
                webglCtx.cacheFontAtlas("Arial", "Bold");
            }
        ...
        processKey(e) {
            ...
            if (e.type == "keydown") {
                ...
                if (e.ctrlKey && e.key === "z" && !this.read_only) {
                    this.undo();
                }

                if (e.ctrlKey && e.key === "y" && !this.read_only) {
                    this.redo();
                }
            }
        ...
        undo() {
            ...
            // 현재
            const currentState = this.graph.serialize();
            this.redoStack.push(this.tempDeepClone(currentState));
            const previousState = this.undoStack.pop();
            this.graph.configure(this.tempDeepClone(previousState));
            this.draw(true, true);
        }
        ...
        redo() {
            ...
            const currentState = this.graph.serialize();
            this.undoStack.push(this.tempDeepClone(currentState));
            const nextState = this.redoStack.pop();
            this.graph.configure(this.tempDeepClone(nextState));
            this.draw(true, true);
        }

        debounceTime(func, wait) {
            let timeout = null;
        ...
        checkDropItem(e) {
            if (e.dataTransfer.files.length) {
                const file = e.dataTransfer.files[0];
                const ext = LGraphCanvas.getFileExtension(
                    file.name
                ).toLowerCase();
        ...
        setZoom(value, zooming_center) {
          ...
          const center = this.convertOffsetToCanvas( zooming_center );
          ...
          const new_center = this.convertOffsetToCanvas( zooming_center );
            ...
            if (subnode.inputs)
                for (let i = 0; i < subnode.inputs.length; ++i) {
                    ...
                    if (this.drawButton(20, y + 2, w - 20, h - 2)) {
                        const type =
                            subnode.constructor.input_node_type ||
                            "graph/input";
                        this.graph.beforeChange();
            ...
            if (subnode.outputs)
                for (let i = 0; i < subnode.outputs.length; ++i) {
                    ...
                    if (this.drawButton(canvas_w - w, y + 2, w - 20, h - 2)) {
                        const type =
                            subnode.constructor.output_node_type ||
                            "graph/output";
                        this.graph.beforeChange();
                        const newnode = LiteGraph.createNode(type);
                        if (newnode) {
        ...
        drawButton(x, y, w, h, text, bgcolor, hovercolor, textcolor) {
            ...
            const hover = LiteGraph.isInsideRectangle(
                pos[0],
                pos[1],
                x,
                y,
            ...
            if (this.graph) {
                ...
                if (
                    ...
                    ctx.imageSmoothingEnabled =
                        ctx.imageSmoothingEnabled = false; // ctx.mozImageSmoothingEnabled =
                    if (
                        !this._bg_img ||
                    ...
                    ctx.globalAlpha = 1.0;
                    ctx.imageSmoothingEnabled =
                        ctx.imageSmoothingEnabled = true; //= ctx.mozImageSmoothingEnabled
                }
        ...
        drawNode(node, ctx) {
            ...
            if (!node.flags.collapsed) {
                ...
                // collapsed 상태에서 연결된 슬롯만 간단히 렌더링

                let inputSlot = null;
                let outputSlot = null;

                // 입력 슬롯 중 연결된 첫 번째 슬롯 탐색
                if (node.inputs) {
                    for (let i = 0; i < node.inputs.length; i++) {
                ...
                // 출력 슬롯 중 연결된 마지막 슬롯 탐색
                if (node.outputs) {
                    for (let i = 0; i < node.outputs.length; i++) {
                        const outSlot = node.outputs[i];
                        if (outSlot.links && outSlot.links.length) {
                            outputSlot = outSlot;
                        }
                    }
                }
                ...
                if (inputSlot) {
                    ...
                    ctx.beginPath();
                    if (
                        inputSlot.type === LiteGraph.EVENT ||
                        inputSlot.shape === LiteGraph.BOX_SHAPE
                    ) {
                        ctx.rect(x - 7 + 0.5, y - 4, 14, 8);
                    } else if (inputSlot.shape === LiteGraph.ARROW_SHAPE) {
                        ctx.moveTo(x + 8, y);
                        ctx.lineTo(x - 4, y - 4);
                        ctx.lineTo(x - 4, y + 4);
                        ctx.closePath();
                ...
                if (outputSlot) {
                    ...
                    ctx.beginPath();
                    if (
                        outputSlot.type === LiteGraph.EVENT ||
                        outputSlot.shape === LiteGraph.BOX_SHAPE
                    ) {
                        ctx.rect(x - 7 + 0.5, y - 4, 14, 8);
                    } else if (outputSlot.shape === LiteGraph.ARROW_SHAPE) {
                        ctx.moveTo(x + 6, y);
                        ctx.lineTo(x - 6, y - 4);
                        ctx.lineTo(x - 6, y + 4);
                        ctx.closePath();
        ...
        drawConnections(ctx) {
            ...
            for (let n = 0, l = nodes.length; n < l; ++n) {
                ...
                for (let i = 0; i < node.inputs.length; ++i) {
                    ...
                    const end_node_slotpos = node.getConnectionPos(
                        true,
                        i,
                        tempB
                    );
                    ...
                    if (
                        !LiteGraph.overlapBounding(link_bounding, margin_area)
                    ) {
                        continue;
                    }
        ...
        showGrpPanel(grp) {
            ...
            const panel = this.createPanel(grp.title || "", {
                closable: true,
                window: ref_window,
                onOpen: function () {
            ...
            function inner_refresh() {
                ...
                const fUpdate = (name, value) => {
                    ...
                    graphcanvas.saveUndoStack(); // Undo 상태 저장
                    switch (name) {
                        case "Title":
                            grp.title = value;
                            break;
                        ...
                        case "Color":
                            if (LGraphCanvas.node_colors[value]) {
                                grp.color =
                                    LGraphCanvas.node_colors[value].groupcolor;
                            } else {
                        ...
                        case "X":
                            grp.move(parseInt(value) - grp.pos[0], 0);
                            break;
                        case "Y":
                ...
                // let nodeCol = "";
                // if (grp.color !== undefined) {
                //   nodeCol = Object.keys(LGraphCanvas.node_colors).filter(function (nK) {
                //     return LGraphCanvas.node_colors[nK].groupcolor == grp.color;
                //   });
                // }
        ...
        drawGuideLines(
            ...
            if (otherRoundWidth === roundWidth) {
                // 선택 그룹
                ctx.beginPath();
                ctx.moveTo(roundX, roundY - 10);
                ctx.lineTo(roundX, roundY + roundHeight + 10);
                ctx.stroke(); // 선을 그리기 위해 stroke 호출

                // 아래쪽
                ctx.beginPath();
                ctx.moveTo(roundX + +roundWidth, roundY - 10);
                ctx.lineTo(roundX + roundWidth, roundY + roundHeight + 10);
                ctx.stroke(); // 선을 그리기 위해 stroke 호출

                // 같은 그룹
                ctx.beginPath();
                ctx.moveTo(otherRoundX, otherRoundY - 10);
                ctx.lineTo(otherRoundX, otherRoundY + otherRoundHeight + 10);
                ctx.stroke(); // 선을 그리기 위해 stroke 호출
            ...
            if (otherRoundHeight === roundHeight) {
                // 선택 그룹
                ctx.beginPath();
                ctx.moveTo(roundX - 10, roundY);
                ctx.lineTo(roundX + roundWidth + 10, roundY);
                ctx.stroke(); // 선을 그리기 위해 stroke 호출

                // 아래쪽
                ctx.beginPath();
                ctx.moveTo(roundX - 10, roundY + roundHeight);
                ctx.lineTo(roundX + roundWidth + 10, roundY + roundHeight);
                ctx.stroke(); // 선을 그리기 위해 stroke 호출

                // 같은 그룹
                ctx.beginPath();
                ctx.moveTo(otherRoundX - 10, otherRoundY);
                ctx.lineTo(otherRoundX + otherRoundWidth + 10, otherRoundY);
                ctx.stroke(); // 선을 그리기 위해 stroke 호출
            ...
            // 수직선
            if (otherRoundX === roundX) {
                if (roundY < otherRoundY) {
                    ctx.beginPath();
                    ctx.moveTo(roundX, roundY);
                    ctx.lineTo(otherRoundX, otherRoundY + otherRoundHeight);
                    ctx.stroke(); // 선을 그리기 위해 stroke 호출
                } else {
                    ctx.beginPath();
                    ctx.moveTo(otherRoundX, otherRoundY);
                    ctx.lineTo(roundX, roundY + roundHeight);
                    ctx.stroke(); // 선을 그리기 위해 stroke 호출
                }
            }

            // 수직선, x + width
            if (otherRoundX + otherRoundWidth === roundX + roundWidth) {
                if (roundY < otherRoundY) {
                    ctx.beginPath();
                    ...
                    ctx.lineTo(roundX + roundWidth, roundY + roundHeight);
                    ctx.stroke(); // 선을 그리기 위해 stroke 호출
                }
            }

            // 수평선
            if (otherRoundY === roundY) {
                if (roundX < otherRoundX) {
                    ctx.beginPath();
                    ctx.moveTo(roundX, roundY);
                    ctx.lineTo(otherRoundX + otherRoundWidth, otherRoundY);
                    ctx.stroke(); // 선을 그리기 위해 stroke 호출
                } else {
                    ctx.beginPath();
                    ctx.moveTo(otherRoundX, otherRoundY);
                    ctx.lineTo(roundX + roundWidth, roundY);
                    ctx.stroke(); // 선을 그리기 위해 stroke 호출
                }
            }

            // 수평선, y + height
            if (otherRoundY + otherRoundHeight === roundY + roundHeight) {
                if (roundX < otherRoundX) {
                    ctx.beginPath();
                    ...
                    ctx.beginPath();
                    ctx.moveTo(otherRoundX, otherRoundY + otherRoundHeight);
                    ctx.lineTo(roundX + roundWidth, roundY + roundHeight);
                    ctx.stroke(); // 선을 그리기 위해 stroke 호출
                }
            }
        ...
        drawGroups(canvas, ctx) {
            ...
            for (let i = 0; i < groups.length; ++i) {
                ...
                const color =
                    group.color || LGraphCanvas.node_colors.black.groupcolor;

                // 배경과 외곽선 스타일 설정
                ctx.fillStyle = color; // 어두운 배경색

                ctx.globalAlpha = 0.75; // 0.75 * this.editor_alpha;
                ctx.beginPath();
                ...
                // 둥근 모서리 상자 그리기
                ctx.fillStyle = color || "#333"; // 기본 배경색
                ctx.beginPath();
                ctx.moveTo(pos[0] + radius, pos[1]);
                ctx.lineTo(pos[0] + size[0] - radius, pos[1]);
                ...
                // 리사이즈 핸들러
                if (!this.read_only) {
                    ctx.fillStyle = "#0085FF"; // 삼각형 색상
                    ctx.beginPath();
                    ctx.moveTo(pos[0] + size[0] - 7, pos[1] + size[1]); // 삼각형의 왼쪽 꼭지점
                    ctx.lineTo(pos[0] + size[0], pos[1] + size[1]); // 삼각형의 오른쪽 꼭지점
                    ctx.lineTo(pos[0] + size[0], pos[1] + size[1] - 7); // 삼각형의 위쪽 꼭지점
                    ctx.closePath();
                    ctx.fill();
                }
                ...
                // 타이틀
                const defaultSize = LiteGraph.DEFAULT_GROUP_FONT_SIZE || 20;
                const font_size = group.font_size || defaultSize;

                ctx.fillStyle = "#ffffff";
                ctx.font = "Bold " + font_size + "px Arial";
                ctx.textAlign = "left";
                ctx.fillText(group.title, pos[0] + 12, pos[1] + font_size + 8);
                ...
                if (!this.read_only) {
                    ...
                    for (let n = 0; n < boxInfo.length; n++) {
                        const testLine = line + boxInfo[n] + "";
                        const metrics = ctx.measureText(testLine);
                        const testWidth = metrics.width;
                        if (testWidth > group.size[0] - 30 && n > 0) {
                ...
                if (
                    this.selected_group &&
                    group?.id !== this.selected_group?.id &&
                    !this.read_only
                ) {
                    ctx.strokeStyle = "#c4c4c4";
                    ctx.lineWidth = 1;

                    const width = this.selected_group?.size[0];
                    const height = this.selected_group?.size[1];
                    const x = this.selected_group?.pos[0];
                    const y = this.selected_group?.pos[1];

                    const otherWidth = group.size[0];
                    const otherHeight = group.size[1];
                    const otherX = group.pos[0];
                    const otherY = group.pos[1];
                    ...
                    // 비교 그룹 위치/크기 반올림
                    const otherRoundX = Math.round(otherX);
                    const otherRoundY = Math.round(otherY);
                    const otherRoundWidth = Math.round(otherWidth);
                    const otherRoundHeight = Math.round(otherHeight);
        ...
        scaleFit() {
            if (this.canvas) {
                ...
                const isLarge = window.innerWidth >= 3840;
                if (isLarge) {
                    this.ds.scale = 2;
                    this.ds.max_scale = 2;
                    this.ds.min_scale = 2;
        ...
        renderLink(
            ...
            if (link) {
                const start_node = this.graph.getNodeById(link.origin_id); // 시작 노드
                const end_node = this.graph.getNodeById(link.target_id); // 끝 노드

                const startStatus = start_node.properties.status;
                const endStatus = end_node.properties.status;

                const resultIndex = start_node.properties["resultIndex"];
                // console.log(resultIndex)

                // link.origin_slot === resultIndex 일때만 색상 변경
                if (
                    typeof resultIndex === "number" &&
                    link.origin_slot === resultIndex
                ) {
                    if (startStatus === 1 && endStatus === 1) {
                        color = "#34c38f";
                    } else if (startStatus === -1 || endStatus === -1) {
                        color = "#f46a6a";
        ...
        drawNodeWidgets(node, posY, ctx, active_widget) {
            ...
            for (let i = 0; i < widgets.length; ++i) {
                ...
                posY +=
                    (w.computeSize ? w.computeSize(widget_width)[1] : H) + 4;
                ctx.globalAlpha = this.editor_alpha;
            }
            ctx.restore();
            ctx.textAlign = "left";
        ...
        static getBoundaryNodes(nodes) {
            ...
            for (const nID in nodes) {
                ...
                if (
                    right === null ||
                    x + width > right.pos[0] + right.size[0]
                ) {
                    right = node;
                }
                if (
        ...
        static showMenuNodeOptionalOutputs(v, _options, e, prev_menu, node) {
            if (!node) {
                return;
            }
            ...
            function inner_clicked(v, options, e) {
                switch (v) {
                    ...
                    case "Delete":
                        ...
                        break;
                    default:
                    /*const nodeCreated = createDefaultNodeForSlot({   nodeFrom: node_left
        ...
        createDefaultNodeForSlot(_optPass) {
            // addNodeMenu for connection
            const optPass = _optPass || {};
            ...
            if (slotX === false || iSlotConn === false) {
                console.warn(
                    "createDefaultNodeForSlot bad slotX " +
                        slotX +
                        " " +
                        iSlotConn
                );
            ...
            if (slotTypesDefault && slotTypesDefault[fromSlotType]) {
                ...
                if (
                    ...
                    Array.isArray(slotTypesDefault[fromSlotType])
                ) {
        ...
        showSearchBox(event, options) {
            ...
            const left =
                (event ? event.clientX : rect.left + rect.width * 0.5) - 80;
            const top =
                (event ? event.clientY : rect.top + rect.height * 0.5) - 20;
            ...
            function select(name) {
                if (name) {
                    if (that.onSearchBoxSelection) {
                        ...
                        const extra =
                            LiteGraph.searchbox_extras[name.toLowerCase()];
                        if (extra) {
                            name = extra.type;
                        }
                        ...
                        if (options.node_from) {
                            ...
                            if (
                                typeof options.node_from.outputs[iS] !==
                                "undefined"
                            ) {
                                if (iS !== false && iS > -1) {
                        ...
                        if (options.node_to) {
                            ...
                            if (
                                typeof options.node_to.inputs[iS] !==
                                "undefined"
                            ) {
                                if (iS !== false && iS > -1) {
                                    // try connection
            ...
            function refreshHelper() {
                ...
                if (that.onSearchBox) {
                    ...
                    for (const i in LiteGraph.searchbox_extras) {
                        ...
                        const ctor =
                            LiteGraph.registered_node_types[extra.type];
                    ...
                    function inner_test_filter(type, _optsIn) {
                        const optsIn = _optsIn || {};
                        const optsDef = {
                            skipFilter: false,
                        ...
                        if (options.do_type_filter && !opts.skipFilter) {
                            ...
                            if (opts.outTypeOverride !== false)
                                _sV = opts.outTypeOverride;
                            //if (_sV.toLowerCase() == "_event_") _sV = LiteGraph.EVENT; // -1

                            if (sOut && _sV) {
                                //console.log("search will check filter against "+_sV);
                                if (
                                    LiteGraph.registered_slot_out_types[_sV] &&
                                    LiteGraph.registered_slot_out_types[_sV]
                                        .nodes
                                ) {
                                    // type is stored
                                    //console.debug("check "+sType+" in "+LiteGraph.registered_slot_out_types[_sV].nodes);
                                    const doesInc =
                                        LiteGraph.registered_slot_out_types[
                                            _sV
                                        ].nodes.includes(sType);
                                    if (doesInc !== false) {
                                        //console.log(sType+" HAS "+_sV);
                                    } else {
                                        /*console.debug(LiteGraph.registered_slot_out_types[_sV]);
                                          console.log(+" DONT includes "+type);*/
                                        return false;
                                    }
                                }
        ...
        createPanel(title, options) {
            ...
            root.close = () => {
                ...
                /* XXX CHECK THIS */
                if (root.parentNode) {
                    root.parentNode.removeChild(this);
                }
            ...
            root.addWidget = (type, name, value, options, callback) => {
                ...
                type = type.toLowerCase();
                if (type == "number") str_value = parseFloat(value).toFixed(3);
                ...
                if (type == "code")
                    ...
                    elem.addEventListener("click", function (e) {
                        ...
                        this.querySelector(".property_value").innerText = this
                            .value
                            ? "true"
                            : "false";
                    ...
                    const inputType = type === "string" ? "string" : "number";
                    ...
                    input.classList.add("property_value_input");
                    if (disabled) {
                    ...
                    input.value =
                        type === "string" ? str_value : Number(str_value);
                    value_element.appendChild(input);
                    ...
                    input.addEventListener("input", function (e) {
                        ...
                        const propname =
                            root.parentNode.dataset["property"] || name;
                        const proptype = root.parentNode.dataset["type"];
                    ...
                    value_element.addEventListener("click", (event) => {
                        event.preventDefault();
                        if (disabled) return;
                  ...
                  <span class='property_value_img'>
                    ...
                    elem.querySelector(".property_name").innerText =
                        options.label || name;
                    const value_element = elem.querySelector(
                        ".property_value_img"
                    );
        ...
        closePanels() {
            ...
            const panel2 = document.querySelector("#option-panel");
            if (panel2) panel2.close();
        }
                    ...
                    switch (name) {
                        /*case "Render mode":
                              ...
                              if (options.values && options.key){
                                  const kV = Object.values(options.values).indexOf(value);
        ...
        showShowNodePanel(node) {
            ...
            function inner_refresh() {
                ...
                const fUpdate = (name, value) => {
                    ...
                    const kV = Object.values(LiteGraph.NODE_MODES).indexOf(
                        value
                    );

                    if (name.includes("inputName")) {
                        const target = node.properties[`editable-${name}`];
                        node.setInputName(target.index, value);
                        node.properties[`editable-${name}`].value = value;
                        // delete node.properties[name];
                    } else if (name.includes("outputName")) {
                        const target = node.properties[`editable-${name}`];
                        node.setOutputName(target.index, value);
                        node.properties[`editable-${name}`].value = value;
                    } else {
                        switch (name) {
                            case "Title":
                            ...
                            case "Color":
                                if (LGraphCanvas.node_colors[value]) {
                                    node.color =
                                        LGraphCanvas.node_colors[value].color;
                                    node.bgcolor =
                                ...
                                break;
                            case "x":
                                node.pos = [parseInt(value), node.pos[1]];
                                break;
                            case "y":
                                node.pos = [node.pos[0], parseInt(value)];
                                break;
                            case "Width":
                                node.size = [parseInt(value), node.size[1]];
                                break;
                            case "Height":
                                node.size = [node.size[0], parseInt(value)];
                                break;
                            default:
                ...
                //     "text",
                //     "상태",
                //     getDisplayName(node.properties.status),
                //     fUpdate,
                //     {
                //         disabled: true,
                //     }
                ...
                const exceptions = ["font_size", "status", "message"];
                for (let pName in node.properties) {
                    if (
                        exceptions.includes(pName) ||
                        pName.includes("editable-")
                    )
                        continue;
                ...
                if (check) {
                    ...
                    for (let pName in node.properties) {
                        if (pName.includes("editable-")) {
                            const value = node.properties[pName].value;
                            const info = node.getPropertyInfo(pName);
                            const type = info.type || "string";
                            const name = pName.split("-")[1];
        ...
        showSubgraphPropertiesDialogRight(node) {
            ...
            if (old_panel) old_panel.close();
            // new panel
            const panel = this.createPanel("Subgraph Outputs", {
                closable: true,
                width: 500,
            });
            panel.node = node;
            ...
            const html =
                " + <span class='label'>Name</span><input class='name'/><span class='label'>Type</span><input class='type'></input><button>+</button>";
            const elem = panel.addHTML(html, "subgraph_property extra", true);
            elem.querySelector(".name").addEventListener(
                "keydown",
                function (e) {
                    if (e.keyCode == 13) {
        ...
        static onMenuNodeColors(value, options, e, menu, node) {
            ...
            function inner_clicked(v) {
                ...
                const color = v.value
                    ? LGraphCanvas.node_colors[v.value]
                    : null;
        ...
        bringGroupToFront(grp) {
            const groups = this.graph._groups;
            const index = this.findGroupIndexById(grp.id);
        ...
        onGrpSizeEditor(item, options, e, menu, grp) {
            const property = item.property || "title";
            ...
            const button = dialog.querySelector(".okBtn");
            button.addEventListener("click", inner);
            canvas.parentNode.appendChild(dialog);

            const width = document.querySelector("#widthValue");
            const height = document.querySelector("#heightValue");
            // if (width) width.focus();

            if (width) {
                width.value = Number(grp.size[0]);
                // input.value = value;
                // width.addEventListener("blur", function (e) {
                //   this.focus();
                // });
            ...
            if (height) {
                height.value = Number(grp.size[1]);
            ...
            function inner() {
                setValue(width?.value, height?.value);
            }

            function setValue(width, height) {
                const _width = width || grp.size[0];
                const _height = height || grp.size[1];
                grp.size = [_width, _height];
                if (dialog.parentNode) {
                    dialog.parentNode.removeChild(dialog);
                }
                grp.setDirtyCanvas(true, true);
            }
        }

        onShowPropertyEditor(item, options, e, menu, node) {
            const input_html = "";
            const property = item.property || "title";
            const value = node[property];
        ...
        onGrpPositionEditor(item, options, e, menu, grp) {
            // TODO refactor :: use createDialog ?
            ...
            function inner() {
                setValue(x?.value, y?.value);
            }

            function setValue(x, y) {
                const _x = x || grp.pos[0];
                const _y = y || grp.pos[1];

                grp.recomputeInsideNodes();
                const delta = [_x - grp.pos[0], _y - grp.pos[1]];
                grp.move(delta[0], delta[1]);
        ...
        addNodeGuideLines(ctx) {
            const nodesList = this.selected_nodes
                ? Object.keys(this.selected_nodes)
                : [];

            if (
                nodesList?.length === 1 &&
                !this.read_only &&
                this.graph._nodes?.length >= 2
            ) {
                const selectedNode = this.selected_nodes[nodesList[0]];

                const title_mode = selectedNode.constructor.title_mode;
                const titleHeight = title_mode
                    ? 0
                    : LiteGraph.NODE_TITLE_HEIGHT;

                const nodeInfo = this.graph._nodes
                    .map((item) => ({
                        x: item.pos[0],
                ...
                const width = selectedNode.size[0];
                const height = selectedNode.size[1];
                const x = selectedNode.pos[0];
                const y = selectedNode.pos[1] - titleHeight;

                ctx.strokeStyle = "#c4c4c4";
                ctx.lineWidth = 1;
                for (let i in nodeInfo) {
                    ctx.strokeStyle = 0xff0000;
                    const otherNode = nodeInfo[i];
    ...
    class ContextMenu {
        constructor(values, options) {
            ...
            if (options.event) {
                ...
                if (options.parentMenu) {
                    const rect =
                        options.parentMenu.root.getBoundingClientRect();
            ...
            // 현재 활성화 캔버스를 찾고 그 부모 엘리먼트를 this.parent에 추가
            const graphcanvas = LGraphCanvas.active_canvas;
            const canvas = graphcanvas.canvas;
            this.parent = canvas.parentElement || document;
        }
            ...
            function inner_onclick(e) {
                ...
                if (value) {
                    ...
                    if (value.submenu) {
                        ...
                        const submenu = new that.constructor(
                            value.submenu.options,
                            {
    ...
    class CurveEditor {
        ...
        onMouseMove(localpos, graphcanvas) {
            ...
            if (s < 0) return;
            const x =
                (localpos[0] - this.margin) / (this.size[0] - this.margin * 2);
            const y =
                (localpos[1] - this.margin) / (this.size[1] - this.margin * 2);
            const curvepos = [
